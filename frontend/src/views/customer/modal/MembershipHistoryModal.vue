<template>
  <BaseModal v-if="open" title="멤버십 상세" @close="emitClose">
    <div class="modal-body">
      <section class="box">
        <div class="summary">
          <div class="row"><div class="k">등급</div><div class="v">{{ membershipView.gradeName || "미가입" }}</div></div>
          <div class="row"><div class="k">상태</div><div class="v">{{ membershipView.membershipStatus || "-" }}</div></div>
          <div class="row"><div class="k">가입일</div><div class="v">{{ fmtDate(membershipView.joinedAt) }}</div></div>
          <div class="row"><div class="k">최근 변경일</div><div class="v">{{ fmtDate(membershipView.calculatedAt) }}</div></div>
        </div>
      </section>
...
const membershipView = computed(() => props.membership || ({
  gradeName: "미가입",
...
const loadHistories = async (page = 1) => {
  if (!fromDate.value || !toDate.value) return; // 백에서 필수
  loading.value = true;
  try {
    const data = await getCustomerMembershipHistoriesApi({
