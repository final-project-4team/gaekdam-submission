...
function normalizeWidget(widget){
  ...
  if (labels.length === 0 && props.period && /^\d{4}$/.test(props.period)){
    ...
    if (!numericData || numericData.length === 0) return { labels: generated, rawData: new Array(12).fill(null), numericData: new Array(12).fill(null) }
...
const ALLOWED_KEYS = ['RESERVATION_COUNT','CANCEL_COUNT','NO_SHOW_COUNT','FACILITY_REVENUE']
function selectFormatter(widget){
  const key = (widget?.widgetKey||'').toString().toUpperCase()
  // Treat facility revenue (and any revenue/income) as currency
  if (key.includes('REVENUE') || key.includes('INCOME') || key.includes('FACILITY_REVENUE')) {
    return (v) => { try { return new Intl.NumberFormat().format(v) } catch (e) { return String(v) } }
  }
  return formatCount
}
...
function pickColor(widget){
  ...
  if (k.includes('REVENUE') || k.includes('INCOME') || k.includes('FACILITY_REVENUE')) return '#0ea5a4' // teal for revenue
  if (k.includes('RESERVATION') || k.includes('BOOKING')) return '#3b82f6' // blue for bookings/reservations
  if (k.includes('CANCEL')) return '#ef4444' // red for cancellations
  if (k.includes('NO_SHOW')) return '#f59e0b' // amber for no-shows
  return '#6366f1'
}

function pickUnitText(widget){
  const k = (widget?.widgetKey||'').toString().toUpperCase()
  if (k.includes('REVENUE') || k.includes('INCOME') || k.includes('FACILITY_REVENUE')) return '원'
  // default to count unit for reservation/cancel/no-show
  return '회'
}

function isCumulativeWidget(widget){
  const k = (widget?.widgetKey||'').toString().toUpperCase()
  // Allow cumulative view for reservation/booking, cancel, no-show and revenue metrics
  return k.includes('RESERVATION') || k.includes('BOOKING') || k.includes('CANCEL') || k.includes('NO_SHOW') || k.includes('REVENUE') || k.includes('INCOME') || k.includes('FACILITY_REVENUE')
}
...
function buildConfig(widget){
  ...
  if (shouldShowTarget){
    ...
    datasets.push({ label:'목표', data:goalData, fill:false, borderColor:'#10b981', backgroundColor:'#10b981', borderDash:[6,4], pointRadius:0, borderWidth:1.5, tension:0, spanGaps:true, isGoal:true })
  }
  ...
  if (isOriginalView){
    try{
      ...
      if (maxIdx>=0){ const maxArr = new Array(len).fill(null); maxArr[maxIdx]=maxVal; datasets.push({ label:'최대', data:maxArr, fill:false, showLine:false, pointRadius:6, pointHoverRadius:8, backgroundColor:'#f59e0b', borderColor:'#f59e0b', spanGaps:true, order:2, showValue:true, valueColor:'#f59e0b' }) }
      if (minIdx>=0 && minIdx!==maxIdx){ const minArr = new Array(len).fill(null); minArr[minIdx]=minVal; datasets.push({ label:'최소', data:minArr, fill:false, showLine:false, pointRadius:6, pointHoverRadius:8, backgroundColor:'#10b981', borderColor:'#10b981', spanGaps:true, order:2, showValue:true, valueColor:'#10b981' }) }
    }catch(e){}
  }

  // plugin for rendering marker values
  const markerValuePlugin = { id:'rev_markerValue', afterDatasetsDraw: (chart)=>{
    try{
      const ctx = chart.ctx
      chart.data.datasets.forEach((ds, dsIndex)=>{
        ...
        meta.data.forEach((el, idx)=>{
          ...
          ctx.save(); ctx.fillStyle = ds.valueColor||ds.borderColor||'#000'; ctx.font='12px sans-serif'; ctx.textAlign='left'; ctx.textBaseline='middle'
          let text = String(v)
  ...
  return { type:'line', data:{ labels:labels, datasets:datasets }, plugins:[markerValuePlugin], options:{ responsive:true, maintainAspectRatio:false, plugins:{ datalabels:{ display:false }, zoom:{ zoom:{ wheel:{ enabled:true }, pinch:{ enabled:true }, mode:'x', drag:{ enabled:!!dragEnabled.value, backgroundColor:'rgba(0,0,0,0.08)' } }, pan:{ enabled:true, mode:'x' } }, tooltip:{ mode:'index', intersect:false, callbacks:{ label:function(context){ const ds = context.dataset||{}; const val = (context.parsed && context.parsed.y!==undefined)?context.parsed.y:context.raw; try{ if (ds.isGoal) return '목표: '+formatter(val); return (ds.label||'')+': '+formatter(val) }catch(e){ return String(val) } } } } }, interaction:{ mode:'index', intersect:false }, scales:{ x:{ display:true }, y:{ display:true, ticks:{ callback:function(value){ try{ return selectFormatter(widget)(value) }catch(e){ return String(value) } } } } } } }
}

function renderChart(){
  if (!chartEl.value) return
  if (!hasData.value){ if (chartInstance){ chartInstance.destroy(); chartInstance=null } return }
  const ctx = chartEl.value.getContext('2d')
  const cfg = buildConfig(props.widget)
  if (chartInstance){ chartInstance.data.labels = cfg.data.labels; chartInstance.data.datasets = cfg.data.datasets; chartInstance.options = cfg.options; chartInstance.update() }
...
function loadSavedMode(widget){ try{ const v = localStorage.getItem(storageKeyForWidget(widget)); if (v!==null) cumulativeMode.value = v==='1' }catch(e){} }

function getDisplaySeriesForExport(widget){ const { labels, rawData, numericData } = normalizeWidget(widget); let displayData = Array.isArray(numericData) ? numericData.slice() : []; if (isCumulativeWidget(widget) && cumulativeMode.value){ let sum=0; displayData = displayData.map(v=>{ const val = (v===null||Number.isNaN(v))?0:v; sum+=val; return sum }); if ((!displayData||displayData.length===0) && (widget && widget.targetValue!==undefined && widget.targetValue!==null)) displayData=[0] } return { labels, rawData, numericData, displayData }
}

function downloadCSV(){ try{ const { labels, rawData, numericData, displayData } = getDisplaySeriesForExport(props.widget); const header=['label','raw','value']; const rows=[]; const n = Math.max((labels||[]).length, (displayData||[]).length, (rawData||[]).length); for (let i=0;i<n;i++){ const lab = (labels && labels[i]!==undefined)?labels[i]:''; const raw=(rawData && rawData[i]!==undefined)?rawData[i]:''; const val=(displayData && displayData[i]!==undefined)?displayData[i]:(numericData && numericData[i]!==undefined?numericData[i]:''); rows.push([`"${String(lab).replace(/"/g,'""') }"`, raw===null?'':String(raw), val===null?'':String(val)]) } const csvContent=[header.join(','), ...rows.map(r=>r.join(','))].join('\n'); const blob=new Blob([csvContent], { type:'text/csv;charset=utf-8;' }); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; const modeSuffix = isCumulativeWidget(props.widget)?(cumulativeMode.value? '_누적':'_원본') : ''; const baseName=(props.widget && (props.widget.widgetKey||props.widget.title))? (props.widget.widgetKey||props.widget.title).toString().replace(/\s+/g,'_') : 'chart'; const name = baseName + modeSuffix + '.csv'; a.download=name; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url) }catch(e){ console.warn('downloadCSV failed', e) } }

function downloadImage(){ if (!chartInstance) return; try{ const url = chartInstance.toBase64Image(); const a=document.createElement('a'); a.href=url; const name=(props.widget && (props.widget.widgetKey||props.widget.title))?((props.widget.widgetKey||props.widget.title).toString().replace(/\s+/g,'_')+'.png'):'chart.png'; a.download=name; document.body.appendChild(a); a.click(); a.remove() }catch(e){ console.warn('downloadImage failed', e) } }
