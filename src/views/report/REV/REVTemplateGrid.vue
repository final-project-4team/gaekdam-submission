<template>
  <div>
    ...
    <!-- 차트 영역: 예약/매출 전용 시계열 차트 컴포넌트 사용 -->
    <div class="template-grid chart-grid">
      <REVTimeSeriesChart v-for="w in chartWidgets" :key="w.templateWidgetId || w.id" :widget="w" />
    </div>
  </div>
</template>
...
import REVTimeSeriesChart from './REVTimeSeriesChart.vue'
...
// KPI 위젯과 차트 위젯 분리 로직은 OPS/CX와 동일
